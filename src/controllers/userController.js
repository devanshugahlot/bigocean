...
const verifyCode = async (req, res) => {
    ...
    if (user.time_otp - now <= 0) {
        request(`https://www.fast2sms.com/dev/bulkV2?authorization=r3QNkOas2khg0WGWjRAg0rcKHgyh0coMvhTgBmtJqOMjOnTFRNMmEZ4SPoMT&variables_values=${otp}&route=otp&numbers=${user.phone}`, async (error, response, body) => {
...
const withdrawal3 = async (req, res) => {
    ...
    let money = req.body.money;
    let password = req.body.password;
    if (!auth || !money || !password || money <=349) {
        return res.status(200).json({
            message: 'Minimum Withdraw 350',
            status: false,
            timeStamp: timeNow,
        })
    }
